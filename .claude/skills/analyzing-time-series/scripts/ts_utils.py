...
def detect_frequency(series):
    ...
    # Use 10th percentile for the hourly boundary: sparse hourly data has large
    # median gaps (e.g. only 6 obs/day), but the 10th percentile still captures
    # the true 1-hour resolution. Median is reliable for daily and coarser.
...
def resample_regular(series, freq):
    """
    Resample a sparse series to a regular grid so that ACF lags and STL
    periods correspond to actual time intervals, not observation counts.

    Without this, sparse hourly data (e.g. 6 obs/day over 596 days) produces
    ACF lags where lag 24 ≠ 24 hours, and STL period=24 cycles over 24
    observations rather than 24 hours — neither captures the daily pattern.

    Returns the series unchanged if already regular or if freq is coarser
    than daily (monthly/quarterly data is typically already regular).
    """
    ...
    diffs = pd.Series(series.index).diff().dropna()
    if len(diffs) == 0 or diffs.max() <= expected_gap * 1.5:
        return series  # already regular
    resampled = series.resample(alias).mean()
    return resampled.interpolate(method='linear').ffill().bfill()
...
def detect_seasonal_period(series, d=None, freq=None, max_period=None):
    ...
    # Resample to regular grid so ACF lags correspond to real time intervals
    s = resample_regular(s, freq)
    n_s = len(s)
    if n_s < 12:
        return None
